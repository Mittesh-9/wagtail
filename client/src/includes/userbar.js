...
exports.Userbar = exports.sortAxeViolations = void 0;
const axe_core_1 = __importDefault(require("axe-core"));
const stimulus_1 = require("@hotwired/stimulus");
const DialogController_1 = require("../controllers/DialogController");
const TeleportController_1 = require("../controllers/TeleportController");
const sortAxeNodes = (nodeResultA, nodeResultB) => {
    if (!nodeResultA || !nodeResultB)
        return 0;
    const nodeA = document.querySelector(nodeResultA.target[0]);
    const nodeB = document.querySelector(nodeResultB.target[0]);
    if (!nodeA || !nodeB)
        return 0;
...
const sortAxeViolations = (violations) => violations.sort((violationA, violationB) => {
...
exports.sortAxeViolations = sortAxeViolations;
class Userbar extends HTMLElement {
    connectedCallback() {
        const template = document.querySelector('#wagtail-userbar-template');
        if (!template)
            return;
        const shadowRoot = this.attachShadow({
            mode: 'open',
        });
        shadowRoot.appendChild(template.content.firstElementChild.cloneNode(true));
        // Removes the template from html after it's being used
        template.remove();
        const userbar = shadowRoot.querySelector('[data-wagtail-userbar]');
        const trigger = userbar === null || userbar === void 0 ? void 0 : userbar.querySelector('[data-wagtail-userbar-trigger]');
        const list = userbar === null || userbar === void 0 ? void 0 : userbar.querySelector('[role=menu]');
    ...
    input:not([disabled])`;
        ...
        const isFocusOnItems = () => shadowRoot.activeElement &&
        ...
        */
        ...
        const handleFocusChange = (event) => {
            ...
            if (!event.relatedTarget ||
                event.relatedTarget.closest('.w-userbar-nav')) {
                return;
            }
    ...
    */
    getAxeConfiguration() {
        var _a;
        const script = (_a = this.shadowRoot) === null || _a === void 0 ? void 0 : _a.querySelector('#accessibility-axe-configuration');
    ...
    initialiseAxe() {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            const accessibilityTrigger = (_a = this.shadowRoot) === null || _a === void 0 ? void 0 : _a.getElementById('accessibility-trigger');
            const config = this.getAxeConfiguration();
            if (!this.shadowRoot || !accessibilityTrigger || !config)
                return;
            ...
            const results = yield axe_core_1.default.run(config.context, config.options);
            const a11yErrorsNumber = results.violations.reduce((sum, violation) => sum + violation.nodes.length, 0);
            ...
            const stimulus = stimulus_1.Application.start(this.shadowRoot.firstElementChild);
            stimulus.register('w-dialog', DialogController_1.DialogController);
            stimulus.register('w-teleport', TeleportController_1.TeleportController);
            const modalReady = new Promise((resolve) => {
                var _a;
                (_a = this.shadowRoot) === null || _a === void 0 ? void 0 : _a.addEventListener('w-dialog:ready', (({ detail, }) => {
                    const { body, dialog } = detail;
                    resolve({ body, dialog });
                }), { once: true, passive: true });
            });
            const { body: modalBody, dialog: modal } = yield modalReady;
            // Disable TS linter check for legacy code in 3rd party `A11yDialog` element
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
            const accessibilityResultsBox = this.shadowRoot.querySelector('#accessibility-results');
            const a11yRowTemplate = this.shadowRoot.querySelector('#w-a11y-result-row-template');
            const a11ySelectorTemplate = this.shadowRoot.querySelector('#w-a11y-result-selector-template');
            const a11yOutlineTemplate = this.shadowRoot.querySelector('#w-a11y-result-outline-template');
            if (!accessibilityResultsBox ||
                !a11yRowTemplate ||
                !a11ySelectorTemplate ||
                !a11yOutlineTemplate) {
                return;
            }
            const innerErrorBadges = this.shadowRoot.querySelectorAll('[data-a11y-result-count]');
            ...
            const showAxeResults = () => {
                ...
                modalBody.innerHTML = '';
                if (results.violations.length) {
                    const sortedViolations = (0, exports.sortAxeViolations)(results.violations);
                    sortedViolations.forEach((violation, violationIndex) => {
                        modalBody.appendChild(a11yRowTemplate.content.cloneNode(true));
                        const currentA11yRow = modalBody.querySelectorAll('[data-a11y-result-row]')[violationIndex];
                        const a11yErrorName = currentA11yRow.querySelector('[data-a11y-result-name]');
                        ...
                        const a11yErrorCount = currentA11yRow.querySelector('[data-a11y-result-count]');
                        a11yErrorCount.textContent = `${violation.nodes.length}`;
                        const a11yErrorContainer = currentA11yRow.querySelector('[data-a11y-result-container]');
                        violation.nodes.forEach((node, nodeIndex) => {
                            a11yErrorContainer.appendChild(a11ySelectorTemplate.content.cloneNode(true));
                            const currentA11ySelector = a11yErrorContainer.querySelectorAll('[data-a11y-result-selector]')[nodeIndex];
                            currentA11ySelector.setAttribute('aria-describedby', a11yErrorName.id);
                            const currentA11ySelectorText = currentA11ySelector.querySelector('[data-a11y-result-selector-text]');
                            const selectorName = node.target[0];
                            // Remove unnecessary details before displaying selectors to the user
                            currentA11ySelectorText.textContent = selectorName.replace(/\[data-block-key="\w{5}"\]/, '');
                            currentA11ySelector.addEventListener('click', () => {
                                var _a, _b;
                                const inaccessibleElement = document.querySelector(selectorName);
                                const a11yOutlineContainer = (_a = this.shadowRoot) === null || _a === void 0 ? void 0 : _a.querySelector('[data-a11y-result-outline-container]');
                                if (a11yOutlineContainer === null || a11yOutlineContainer === void 0 ? void 0 : a11yOutlineContainer.firstElementChild) {
                                    a11yOutlineContainer.removeChild(a11yOutlineContainer.firstElementChild);
                                }
                                a11yOutlineContainer === null || a11yOutlineContainer === void 0 ? void 0 : a11yOutlineContainer.appendChild(a11yOutlineTemplate.content.cloneNode(true));
                                const currentA11yOutline = (_b = this.shadowRoot) === null || _b === void 0 ? void 0 : _b.querySelector('[data-a11y-result-outline]');
                                if (!this.shadowRoot ||
                                    !inaccessibleElement ||
                                    !currentA11yOutline ||
