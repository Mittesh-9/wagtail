 ...
 * </button>
 */
class ProgressController extends stimulus_1.Controller {
    /**
    ...
    static afterLoad(identifier, application) {
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document
                .querySelectorAll(`.${DEFAULT_CLASS}:not([${controllerAttribute}~='${identifier}'])`)
                .forEach((button) => {
                // set the controller attribute, appending to existing if present
                button.setAttribute(controllerAttribute, [button.getAttribute(controllerAttribute) || '', identifier]
                    .filter(Boolean)
                    .join(' '));
                // set the action attribute, appending to existing if present
                button.setAttribute(actionAttribute, [
                ...
                if (activeText) {
                    button.setAttribute(`data-${identifier}-active-value`, activeText);
...
ProgressController.classes = ['active'];
ProgressController.targets = ['label'];
ProgressController.values = {
