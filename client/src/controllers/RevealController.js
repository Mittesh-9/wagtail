...
exports.RevealController = void 0;
 ...
 * </section>
 */
class RevealController extends stimulus_1.Controller {
    connect() {
    ...
    closedValueChanged(shouldClose, previouslyClosed) {
        if (previouslyClosed === shouldClose)
            return;
        ...
        if (!isPeeking)
            this.updateToggleIcon(shouldClose);
        if (shouldClose) {
    ...
    get toggles() {
        const toggles = this.contentTargets
            ...
            .flatMap((id) => Array.from(document.querySelectorAll(`[aria-controls~="${id}"]`)))
    ...
    updateToggleIcon(isOpenIcon = false) {
        if (!this.hasCloseIconClass || !this.hasOpenIconClass)
            return;
        const closeIconClass = this.closeIconClass;
        const openIconClass = this.openIconClass;
        if (closeIconClass === openIconClass)
            return;
        this.toggles
            .map((toggle) => {
            const iconElement = toggle.querySelector('.icon');
            const useElement = iconElement === null || iconElement === void 0 ? void 0 : iconElement.querySelector('use');
            if (!useElement || !iconElement)
                return [];
            return [iconElement, useElement];
        })
    ...
    disconnect() {
        var _a;
        (_a = this.cleanUpPeekListener) === null || _a === void 0 ? void 0 : _a.call(this);
    }
}
exports.RevealController = RevealController;
RevealController.classes = [
    'closed',
    'closeIcon',
    'initial',
    'opened',
...
RevealController.targets = ['content', 'toggle'];
RevealController.values = {
